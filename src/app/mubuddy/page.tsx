...
import { Bot, Send, User, Sparkles, MessageCircle, Coffee, Building, Theater, MapPin, Star } from "lucide-react"
...
function FormattedMessage({ content }: { content: string }) {
  ...
  const renderInlineFormatting = (text: string) => {
    ...
    const regex = /\[([^\]]+)\]\((https?:\/\/[^\)]+)\)|(Rating:\s*(\d(?:\.\d)?)\s*\/\s*5)|\*\*([^*]+)\*\*|\*([^*]+)\*|(₹\d+(?:,\d+)*)/gi;
    ...
    while ((match = regex.exec(text)) !== null) {
      ...
      if (linkText && linkUrl) {
        ...
        elements.push(<StarRating key={`rating-${lastIndex}`} rating={parseFloat(ratingValue)} />);
      } else if (boldContent) {
        elements.push(<strong key={`bold-${lastIndex}`} className="font-semibold">{boldContent}</strong>);
      } else if (italicContent) {
        elements.push(<em key={`italic-${lastIndex}`} className="italic">{italicContent}</em>);
      } else if (price) {
        elements.push(
  ...
  lines.forEach((line, index) => {
    ...
    if (trimmedLine.startsWith('### ')) {
      flushList();
      elements.push(<h3 key={`h3-${index}`} className="text-lg font-semibold mt-4 mb-2">{renderInlineFormatting(trimmedLine.substring(4))}</h3>);
    } else if (trimmedLine.startsWith('## ')) {
      flushList();
      elements.push(<h2 key={`h2-${index}`} className="text-xl font-bold mt-5 mb-3">{renderInlineFormatting(trimmedLine.substring(3))}</h2>);
    } else if (trimmedLine.startsWith('# ')) {
      flushList();
      elements.push(<h1 key={`h1-${index}`} className="text-2xl font-bold mt-6 mb-4">{renderInlineFormatting(trimmedLine.substring(2))}</h1>);
    } else {
